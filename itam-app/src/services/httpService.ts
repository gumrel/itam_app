...
class HttpService implements IHttpService {
	...
	public post<T, U>(url: string, body: U, options: AxiosRequestConfig = {}): Promise<T> {
		return this.request<T, U>(HttpMethod.POST, url, body, options);
	}

	public put<T, U>(url: string, body: U, options: AxiosRequestConfig = {}): Promise<T> {
		return this.request<T, U>(HttpMethod.PUT, url, body, options);
	}

	public patch<T, U>(url: string, body: U, options: AxiosRequestConfig = {}): Promise<T> {
		return this.request<T, U>(HttpMethod.PATCH, url, body, options);
	}

	public delete<T>(url: string, options: AxiosRequestConfig = {}): Promise<T> {
		return this.request<T, undefined>(HttpMethod.DELETE, url, undefined, options);
	}
}
